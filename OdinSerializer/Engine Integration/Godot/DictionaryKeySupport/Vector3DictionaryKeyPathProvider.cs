...
namespace OdinSerializer
{
    ...
    /// <summary>
    /// Dictionary key path provider for <see cref="Vector3"/>
    /// </summary>
    public sealed class Vector3DictionaryKeyPathProvider : BaseDictionaryKeyPathProvider<Vector3>
    {
        ...
        public override Vector3 GetKeyFromPathString(string pathStr)
        {
            ...
            return new Vector3(ParseInvariant(x), ParseInvariant(y), ParseInvariant(z));
        }

        // The write side emits invariant "R" text with '.' replaced by ',', so parsing
        // must reverse that replacement and stay culture-invariant (float.Parse with the
        // current culture breaks on machines where ',' is the thousands separator).
