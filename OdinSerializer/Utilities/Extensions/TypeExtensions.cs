...
public static class TypeExtensions
{
    ...
    private static readonly List<Type> GenericConstraintsSatisfactionTypesToCheckToAdd = new();
    ...
    private static readonly Type[] TwoLengthTypeArrayCached = new Type[2];

    private static readonly Stack<Type> GenericArgumentsContainsTypesArgsToCheckCached = new();

    private static readonly HashSet<string> ReservedCSharpKeywords =
    [
        "abstract",
        "as",
        "base",
    ...
    private static readonly object CachedNiceNamesLock = new();
    ...
    private static string CreateNiceName(Type type)
    {
        ...
        if (type.InheritsFrom(typeof(Nullable<>))) return type.GetGenericArguments()[0].GetNiceName() + "?";

        if (type.IsByRef) return "ref " + type.GetElementType()!.GetNiceName();

        if (type.IsGenericParameter || !type.IsGenericType) return TypeNameGauntlet(type);
    ...
    {
        if (from.IsEnum) return Enum.GetUnderlyingType(from).IsCastableTo(to);

        if (to.IsEnum) return Enum.GetUnderlyingType(to).IsCastableTo(from);

        if ((from.IsPrimitive || from == VoidPointerType) && (to.IsPrimitive || to == VoidPointerType))
        {
            if (requireImplicitCast) return PrimitiveImplicitCasts[from].Contains(to);

            if (from == typeof(IntPtr))
            {
                if (to == typeof(UIntPtr))
                    return false;
                if (to == VoidPointerType) return true;
            }
    ...
    public static bool IsValidIdentifier(string identifier)
    {
        ...
        if (ReservedCSharpKeywords.Contains(identifier)) return false;

        if (!IsValidIdentifierStartCharacter(identifier[0])) return false;
    ...
    public static Func<object, object> GetCastMethodDelegate(this Type from, Type to, bool requireImplicitCast = false)
    {
        ...
        lock (WeaklyTypedTypeCastDelegatesLock)
        {
            if (WeaklyTypedTypeCastDelegates.TryGetInnerValue(from, to, out result) == false)
            {
                ...
                if (method != null) result = obj => method.Invoke(null, new[] { obj });
    ...
    public static Func<TFrom, TTo> GetCastMethodDelegate<TFrom, TTo>(bool requireImplicitCast = false)
    {
        ...
        lock (StronglyTypedTypeCastDelegatesLock)
        {
            if (StronglyTypedTypeCastDelegates.TryGetInnerValue(typeof(TFrom), typeof(TTo), out del) == false)
            {
                ...
                if (method != null) del = Delegate.CreateDelegate(typeof(Func<TFrom, TTo>), method);
    ...
    private static bool QuaternionEqualityComparer(Quaternion a, Quaternion b)
    {
        ...
        return Math.Abs(a.X - b.X) < float.Epsilon && Math.Abs(a.Y - b.Y) < float.Epsilon &&
               Math.Abs(a.Z - b.Z) < float.Epsilon && Math.Abs(a.W - b.W) < float.Epsilon;
#endif
    }
    ...
    /// <remarks>
    ///     <para>
    ///         Note that in the special case of the type <see cref="Godot.Quaternion" />, a special equality comparer
        ...
        Type openGenericInterfaceType)
    {
        ...
        if ((openGenericInterfaceType == GenericListInterface ||
            ...
            candidateType.IsArray) return new[] { candidateType.GetElementType() };
    ...
    public static MethodInfo GetOperatorMethod(this Type type, Operator op, Type leftOperand, Type rightOperand)
    {
        ...
        var types = TwoLengthTypeArrayCached;
    ...
    public static IEnumerable<MemberInfo> GetAllMembers(this Type type, BindingFlags flags = BindingFlags.Default)
    {
        ...
        {
            foreach (var member in currentType.GetMembers(flags)) yield return member;
        }
        else
        {
    ...
    public static IEnumerable<Type> GetBaseTypes(this Type type, bool includeSelf = false)
    {
        ...
        if (includeSelf && type.IsInterface) result.Concat(new[] { type });
        return result;
    }
    ...
    private static string TypeNameGauntlet(this Type type)
    {
        ...
        if (TypeNameAlternatives.TryGetValue(typeName, out altTypeName)) typeName = altTypeName;
    ...
    public static IEnumerable<T> GetCustomAttributes<T>(this Type type, bool inherit) where T : Attribute
    {
        ...
        for (var i = 0; i < attrs.Length; i++) yield return attrs[i] as T;
    }
    ...
    public static bool InheritsFrom(this Type type, Type baseType)
    {
        ...
        if (type.IsInterface && baseType.IsInterface == false) return false;
        ...
        while (t != null)
        {
            ...
            if (baseType.IsGenericTypeDefinition && t.IsGenericType &&
                t.GetGenericTypeDefinition() == baseType) return true;
        ...
        if (methodParams.Length == paramTypes.Count)
        {
            for (var i = 0; i < methodParams.Length; i++)
                if (inherit && paramTypes[i].InheritsFrom(methodParams[i].ParameterType) == false)
                    return false;
                else if (methodParams[i].ParameterType != paramTypes[i]) return false;
            return true;
        }
        ...
        params Type[] knownParameters)
    {
        ...
        if (knownParameters == null) throw new ArgumentNullException("knownParameters");
        ...
        {
            var matches = GenericConstraintsSatisfactionInferredParameters;
            matches.Clear();

            var typesToCheck = GenericConstraintsSatisfactionTypesToCheck;
            typesToCheck.Clear();

            var typesToCheck_ToAdd = GenericConstraintsSatisfactionTypesToCheckToAdd;
            typesToCheck_ToAdd.Clear();

            for (var i = 0; i < knownParameters.Length; i++) typesToCheck.Add(knownParameters[i]);

            var definitions = genericTypeDefinition.GetGenericArguments();

            if (!genericTypeDefinition.IsGenericTypeDefinition)
            {
                var constructedParameters = definitions;
            ...
            foreach (var typeArg in definitions)
            {
                ...
                foreach (var constraint in constraints)
                {
                    ...
                    foreach (var type in typesToCheck_ToAdd) typesToCheck.Add(type);
            ...
            if (matches.Count == definitions.Length)
            {
                ...
                for (var i = 0; i < definitions.Length; i++) inferredParams[i] = matches[definitions[i]];

                if (AreGenericConstraintsSatisfiedBy(genericTypeDefinition, inferredParams)) return true;
            }
    ...
    {
        if (genericMethod == null) throw new ArgumentNullException("genericMethod");
    ...
    {
        if (definitions.Length != parameters.Length) return false;

        lock (GenericConstraintsSatisfactionLock)
        {
            var resolvedMap = GenericConstraintsSatisfactionResolvedMap;
            resolvedMap.Clear();
            ...
            for (var i = 0; i < definitions.Length; i++)
            {
                ...
                if (!definition.GenericParameterIsFulfilledBy(parameter, resolvedMap)) return false;
            }
    ...
    {
        if (genericParameterDefinition == null) throw new ArgumentNullException("genericParameterDefinition");

        if (parameterType == null) throw new ArgumentNullException("parameterType");

        if (resolvedMap == null) throw new ArgumentNullException("resolvedMap");

        if (genericParameterDefinition.IsGenericParameter == false &&
            genericParameterDefinition == parameterType) return true;

        if (genericParameterDefinition.IsGenericParameter == false) return false;
        ...
        var specialConstraints = genericParameterDefinition.GenericParameterAttributes;
        ...
        var constraints = genericParameterDefinition.GetGenericParameterConstraints();
            ...
            else if (constraint.IsClass || constraint.IsInterface || constraint.IsValueType)
            {
                if (constraint.IsGenericType)
                {
                    ...
                    for (var j = 0; j < constraintParams.Length; j++)
                    {
                        ...
                        // Replace resolved constraint parameters with their resolved types
                        if (c.IsGenericParameter && resolvedMap.ContainsKey(c)) c = resolvedMap[c];

                        if (c.IsGenericParameter)
                        {
                            if (!processedParams.Contains(c) &&
                                !GenericParameterIsFulfilledBy(c, p, resolvedMap, processedParams)) return false;
                        }
    ...
    public static bool GenericArgumentsContainsTypes(this Type type, params Type[] types)
    {
        ...
        var argsToCheck = GenericArgumentsContainsTypesArgsToCheckCached;

        lock (argsToCheck)
        {
            argsToCheck.Clear();

            for (var i = 0; i < args.Length; i++) argsToCheck.Push(args[i]);
    ...
    public static bool IsFullyConstructedGenericType(this Type type)
    {
        ...
        if (type.HasElementType)
        {
            ...
            if (element.IsGenericParameter || element.IsFullyConstructedGenericType() == false) return false;
        }
        ...
        for (var i = 0; i < args.Length; i++)
        {
            ...
            if (arg.IsGenericParameter)
                return false;
            if (!arg.IsFullyConstructedGenericType()) return false;
        }
